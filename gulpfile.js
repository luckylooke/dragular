...
gulp.task('build', function() {
  ...
  sequence(['minifyClean', 'styles'], 'build:docs');
});
...
/*
* single, clean concatenated file
 */

gulp.task('concatClean', function () {

  var dragularOrder = [
    'src/dragularPrefix.js',
    'src/dragularModule.js',
    'src/dragularService.js',
    'src/dragularDirective.js',
    'src/dragularSuffix.js'
  ];

  return gulp.src(dragularOrder)
    .pipe(sourcemaps.init())
    .pipe(concat('dragular.js', {newLine: '\n\n'}))
    .pipe(sourcemaps.write('./'))
    .pipe(gulp.dest('./dist'));

});

gulp.task('minifyClean', function() {

  return gulp.src('dist/dragular.js')
    .pipe(gulpif(config.isProd, uglify({
      compress: { drop_console: true }
    })))
    .pipe(gulpif(config.isProd, rename({
